mod tests;
use crate::helpers;
use crate::spf::{Spf, SpfErrorType};
#[allow(dead_code)]
...
#[allow(dead_code)]
pub(crate) fn check_start_of_spf(spf_string: &str) -> Result<(), SpfErrorType> {
...
#[allow(dead_code)]
pub(crate) fn validate_rfc4408(spf: &mut Spf) -> Result<&Spf, SpfErrorType> {
    ...
    if spf.from_src {
        ...
        if !helpers::spf_has_consecutive_whitespace(&spf.to_string()) {
            return Err(SpfErrorType::WhiteSpaceSyntaxError);
        }
    } else {
