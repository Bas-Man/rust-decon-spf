...
pub(crate) fn capture_matches(string: &str, kind: Kind) -> Option<MechanismImpl<String>> {
    lazy_static! {
    ...
    match caps {
        ...
        Some(caps) => {
            ...
            if caps.name("mechanism").is_some() {
                ...
                mechanism = MechanismImpl::new(
                    kind,
                    qualifier_result,
                    Some((*mechanism_string).to_string()),
                );
            } else {
                mechanism = MechanismImpl::new(kind, qualifier_result, None);
            }
            Some(mechanism)
        }
    }
...
// builds a string representation of of the mechanisms stored in the Vec<MechanismImpl<String>>
pub(crate) fn build_spf_str(str: Option<&Vec<MechanismImpl<String>>>) -> String {
...
// builds a string representation of of the mechanisms stored in the Vec<MechanismImpl<IpNetwork>>
pub(crate) fn build_spf_str_from_ip(str: Option<&Vec<MechanismImpl<IpNetwork>>>) -> String {
    let mut partial_spf = String::new();
    for i in str.unwrap().iter() {
        partial_spf.push(' ');
        partial_spf.push_str(i.to_string().as_str());
